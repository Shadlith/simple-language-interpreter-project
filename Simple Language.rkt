#lang racket

; Group 8 - Suvaion Das, Remy Niman, Faraz Mamaghani
; Simple Language Interpreter

; We weren't able to get polymorphism working fully. We did reverse the field value list and do find the index correctly.
; The main problem we have is that we can't both "super" and "this" to work at the same time. 

; We know that we need fields to pull from the compile_type and methods to pull from the function's class type. 
; To do this, we have the compile_type set to change depending on what class you're in and for method calls, we use the class
; that is stored in the instance closure in the state (which is set to the class that is stored in the method's closure). 

; We know that "super" needs to access methods and fields from the super class and higher. We can get that to work, but then lookups for 
; "this" break. If we get lookups for "this" to work, then "super" breaks into an infinte loop.  

; Right now, Test 7 returns 3 instead of 26 and that is because we have "super" functioning properly, but don't have "this" functioning 
; properly. The code does what we want until it reaches the "return this.m2();" line in A's class. At that point, we want it to start the
; search for m2 in Class C (b/c that is the type of "this"). Then, it will not find m2 in Class C and will move on to Class B's methods
; and hit m2 in Class B (at this point, the compile_type would be set to B). 
; Then, it would search for the field x (starting the search in Class B) and not find it until it hit the x from Class A and find 1. 
; Then, it would search for the field y (startnig the search in Class B) and find 22.
; Then, it would serach for the field z (starting the search in Class B) and find 3. 

; The rest of the tests we think would work if we got "super" and "this" to work simultaneously. 

(require "classParser.rkt")

(define parse
  (lambda (filename)
    (parser filename)
    ))

(define interpret
  (lambda (filename classname)
    (define parsed (parse filename))
    (print parsed);just to see what it outputs
    (newline)
    (let ((x (M_state_inital_class_runner parsed '() '())))
    (answer_converter (M_state_lookup 'return x (M_state_run_main (list (get_element 0 x) (get_element 1 x)) classname) '() '() '() '()))) 
    ))

;For Part 3, we had the state have 4 elements:
; Element 0 = declared_list
; Element 1 = value_list
; Element 2 = func_name_list
; Element 3 = func_closure_list

; the function closure contains:
;     formal parameter list
;     the function body
;     a function that creates the function environment from the current environment.
;     what class is this a function of (new for part 4)

; For Part 4, we're going to have 2 "state" variables variables:
; "class_info" and "state"
; "class_info" contains the following (and is made once and never changed): 
;     Element 0 = class_name_list
;     Element 1 = class_closure_list
; "state" containst he following: 
; You add a layer when you hit a curly bracket.
; Layer 0: Element 0 = declared_list
;          Element 1 = value_list (which will either be a value or an instance closure)
; Layer 1: (same format as Layer 0)

; For Part 4, the method closure contains:
;     formal parameter list (Add "this" as an additional parameter to the parameter list in each (non-static) function's closure.)
;     the body of the method
;     a function that creates the method environment from the current environment.
;     a function (or equivalent information) so that you can look up the function's class in the environment/state.
;           this is the compile time type of "this" in the method body. 

; runs evaluate on the body of the main function
(define M_state_run_main
  (lambda (class_info mains_class_name)
    (call/cc (lambda (return) (evaluate
                               (M_state_find_main (M_state_find_class_closure class_info mains_class_name))
                               class_info
                               (list(list(list(box 'return))) (list(list(box 'null))) '(()) '(()))
                               mains_class_name '() return '() '())))
    ))

    
; this outputs the class closure given a class name
(define M_state_find_class_closure
  (lambda (class_info class_name)
    (cond
      ((eq? class_name (car (get_element 0 class_info))) (car (get_element 1 class_info)))
      (else (M_state_find_class_closure (list (cdr (get_element 0 class_info)) (cdr (get_element 1 class_info))) class_name))
      )))

; this outputs the main functions body, given the class closure that has main in it
(define M_state_find_main
  (lambda (class_closure)
    (cond
      ((eq? 'main (car (get_element 3 class_closure))) (get_element 1 (car (get_element 4 class_closure))))
      (else (M_state_find_main (list (get_element 0 class_closure)
                                   (get_element 1 class_closure)
                                   (get_element 2 class_closure)
                                   (cdr (get_element 3 class_closure))
                                   (cdr (get_element 4 class_closure)))))
          )))

; this outputs the class name for a certain method name (this probably only will work for finding main)
    ; in all other cases, we'll know the class of the object we're calling the method on. 
(define M_state_find_method
  (lambda (method_name class_info)
    (cond
      ((null? class_info) (error "function not found"))
      ((member? method_name (car (get_element 1 class_info))) (car (get_element 0 class_info)))
      (else M_state_find_method method_name (cdr class_info))
      )))

(define M_state_inital_class_runner
  (lambda (lis class_name_list class_closure_list)
    (cond
      ((null? lis) (list class_name_list class_closure_list))
      (else (let ((updated_name_list (cons (get_element 1 (car lis)) class_name_list))
                  (updated_closure_list
                   (cons
                    (M_state_class_closure_maker (car lis) (get_element 1 (car lis)) (list class_name_list class_closure_list) '(() (()) (()) (()) (())))
                    class_closure_list)))
             (M_state_inital_class_runner (cdr lis) updated_name_list updated_closure_list))) 
      )))

                                                    
; For Part 4, the class closure contains:
;     super class
;     list of instance field names
;     list of instance values 
;     list of methods/function names
;     list of methods/function closures

; this makes a singular class closure with 5 elements in it
(define M_state_class_closure_maker
  (lambda (lis class_name class_info output)
    (cond
      ((null? lis) output)
      ; if this class does extend another class
      ((and (eq? 'class (car lis)) (not (null? (get_element 2 lis)))) 
       (let ((super_class_closure (M_state_get_class_closure_info (get_element 1 (get_element 2 lis)) class_info )))
       (M_state_class_closure_maker (get_element 3 lis) class_name class_info (list
                                                (get_element 1 (get_element 2 lis))
                                                (get_element 1 super_class_closure)
                                                (get_element 2 super_class_closure)
                                                (get_element 3 super_class_closure)
                                                (get_element 4 super_class_closure)))))
      ; if this class does NOT extend another class
      ((eq? 'class (car lis)) 
       (M_state_class_closure_maker (get_element 3 lis) class_name class_info output))

      ; if we're on a line that declares an instance field
      ((eq? 'var (caar lis))
       (M_state_class_closure_maker (cdr lis) class_name class_info (list
                                               (get_element 0 output)
                                               (cons (get_element 1 (car lis)) (get_element 1 output))
                                               (cons (get_element 2 (car lis)) (get_element 2 output))
                                               (get_element 3 output)
                                               (get_element 4 output))))

      ; if we're on a line for a function, then add the name to the function list and function closure. Note that we add "this" to the function closure here
      ((eq? 'function (caar lis))
       (M_state_class_closure_maker (cdr lis) class_name class_info (list
                                               (get_element 0 output)
                                               (get_element 1 output)
                                               (get_element 2 output)
                                               (cons (get_element 1 (car lis)) (get_element 3 output))
                                               (cons
                                                      (list (cons 'this (get_element 2 (car lis))) (get_element 3 (car lis)) 1 class_name) ;<- 1 is hard-coded b/c we think that we'll always have to pull from layer 1 since we don't have to deal with nested functions
                                                      (get_element 4 output)) 
                                                      
                                                )))
      
      ((eq? 'static-function (caar lis))
       (M_state_class_closure_maker (cdr lis) class_name class_info (list
                                               (get_element 0 output)
                                               (get_element 1 output)
                                               (get_element 2 output)
                                               (cons (get_element 1 (car lis)) (get_element 3 output))
                                               (cons
                                                      (list (get_element 2 (car lis)) (get_element 3 (car lis)) 1 class_name)
                                                      (get_element 4 output)))
                                               ))
      
      )))

                    
(define M_state_get_class_closure_info
  (lambda (class_name class_info)
    (cond
      ((null? (get_element 0 class_info)) (error "class doesn't exist"))
      ((eq? class_name (car (get_element 0 class_info))) (car (get_element 1 class_info)))
      (else (M_state_get_class_closure_info class_name (list (cdr (get_element 0 class_info)) (cdr (get_element 1 class_info)))))
      )))

; For Part 4, the instance closure contains:
;     run-time type of the instance
;     the values of all instance fields

; "lis" is just the list that contains the "new". We want to output a state with an updated value_list where the "value" for the variable
; mapped to this Object is the instance closure. 
(define M_state_instance_closure_maker
  (lambda (class_name class_info)
    (let ((x (M_state_get_class_closure_info class_name class_info)))
      (list class_name (reverse* (box_elements (get_element 2 x)))))
      ))

(define reverse*
  (lambda (lis)
    (cond
      ((null? lis) lis)
      ((list? (car lis)) (append (reverse* (cdr lis)) (list (reverse*(car lis)))))
      (else (append (reverse* (cdr lis)) (list (car lis))))
      )))

(define box_elements
  (lambda (lis)
    (cond
      ((null? lis) lis)
      ((list? (car lis)) (cons (box_elements (car lis)) (box_elements (cdr lis))))
      (else (cons (box (car lis)) (box_elements (cdr lis))))
      )))

(define alias_finder
  (lambda (object_name original_state state)
    (cond
      ((list? object_name) object_name)
      ((eq? 'super object_name) 'super)
      ((and (list? (car (get_element 0 state))) (member? object_name (car (get_element 0 state))))
       (alias_finder object_name original_state (list
                                                 (car (get_element 0 state))
                                                 (car (get_element 1 state))
                                                 (get_element 2 state)
                                                 (get_element 3 state))))
      ((list? (car (get_element 0 state)))
       (alias_finder object_name original_state (list
                                                 (cdr (get_element 0 state))
                                                 (cdr (get_element 1 state))
                                                 (get_element 2 state)
                                                 (get_element 3 state))))
                                                 
      ((and (eq? object_name (unbox (car (get_element 0 state)))) (list? (unbox (car (get_element 1 state))))) object_name)
      ((eq? object_name (unbox (car (get_element 0 state))))
       (alias_finder (unbox (car (get_element 1 state))) original_state original_state))
      (else (alias_finder object_name original_state (list
                                                      (cdr (get_element 0 state))
                                                      (cdr (get_element 1 state))
                                                       (get_element 2 state)
                                                       (get_element 3 state))))
      )))
                                                     

(define M_state_function_closure_maker
  (lambda (lis class_info state)
    (cond
      ((null? lis) state)
      
      ((eq? 'function (caar lis))
       (M_state_function_closure_maker (cdr lis) class_info 
                              (list (get_element 0 state) (get_element 1 state)
                              (M_state_add_to_func_name_list (get_element 2 state) (cadar lis))
                              (M_state_add_to_func_closure_list (get_element 3 state)
                                                                (list(get_element 2 (car lis)) (get_element 3 (car lis)) (length (get_element 0 state)))))))

      ; global parameter reading, "evaluate" comes back up here if it hits the word "function"
      ; the empty list for "compile_type" may be wrong. We're not certain this function even runs anymore 
     ((or (eq? 'var (caar lis)) (eq? '= (caar lis))) (evaluate lis state '() '()'() '()' ()))
      )))

(define answer_converter
  (lambda (var)
    (cond
      ((and (boolean? var) var) 'true)
      ((boolean? var) 'false)
      (else var))))

(define M_state_get_method_closure
  (lambda (func_name func_name_list func_closure_list)
    (cond
      ((null? func_name_list) (error "function not found"))
      ((and (list? (car func_name_list)) (member? func_name (car func_name_list)))
       (M_state_get_method_closure func_name (car func_name_list) (car func_closure_list)))
      ((eq? func_name (car func_name_list)) (car func_closure_list))
      (else (M_state_get_method_closure func_name (cdr func_name_list) (cdr func_closure_list)))
      )))

(define M_value_call_function
 (lambda (func_name class_name object_name actual_params class_info state instance_closure compile_type return break try)
   (let ((x (M_state_get_method_closure
            func_name
            (get_element 3 (M_state_find_class_closure class_info class_name))
            (get_element 4 (M_state_find_class_closure class_info class_name)))))
   (call/cc (lambda (return1) (evaluate (get_element 1 x) class_info
                                        (list
                                         (M_state_add_parameters_to_state
                                          (get_element 0 x)
                                          (M_state_add_row_to_declared_list (get_element 0 state)))
                                         (M_state_add_parameters_to_state
                                          (cons
                                           (M_state_lookup (alias_finder object_name state state) class_info state instance_closure compile_type break try)
                                           (parameter_translator actual_params class_info state instance_closure compile_type break try))
                                          (M_state_add_row_to_value_list (get_element 1 state)))
                                         '()
                                         '())
                                        
                                          (if (eq? object_name 'super)
                                              (get_element 0 (M_state_find_class_closure class_info compile_type)) compile_type)
                                          ;(if (eq? object_name 'super) instance_closure (M_state_lookup (alias_finder object_name state state) class_info state instance_closure break try))
                                          (M_state_lookup (alias_finder object_name state state) class_info state instance_closure compile_type break try)
                                          return1 break try))))
   ))

(define parameter_translator
  (lambda (actual_params class_info state instance_closure compile_type break try)
    (cond
      ((null? actual_params) '())
      ((or (boolean? (car actual_params)) (number? (car actual_params))) (cons (car actual_params) (parameter_translator (cdr actual_params) class_info state instance_closure compile_type break try)))
      ; math expression
      ((and (list? (car actual_params)) (operator? (car actual_params))) (cons (M_value (car actual_params) class_info state instance_closure compile_type break try) (parameter_translator (cdr actual_params) class_info state instance_closure compile_type break try)))
      ; boolean expression 
      ((and (list? (car actual_params)) (boolean_operator? (car actual_params)))
       (cons (M_boolean (car actual_params) class_info state instance_closure compile_type break try) (parameter_translator (cdr actual_params) class_info state instance_closure compile_type break try)))
      ; method call without actual params
      ((and (and (list? (car actual_params)) (eq? 2 (length (car actual_params)))) (eq? 'funcall (get_element 0 (car actual_params))))
       (cons (call/cc (lambda (return) (M_value_call_function
                                  (get_element 2 (get_element 1 (car actual_params)))
                                  (M_state_lookup (alias_finder (get_element 1 (get_element 1 (car actual_params))) state state) class_info state instance_closure compile_type break try)
                                  (get_element 1 (get_element 1 (car actual_params)))
                                  '()
                                  class_info state instance_closure compile_type return break try))) (parameter_translator (cdr actual_params) class_info state instance_closure compile_type break try)))
      ; method call with actual params
      ((and (list? (car actual_params)) (and (eq? 'funcall (get_element 0 (car actual_params)))))
       (cons (call/cc (lambda (return) (M_value_call_function
                                  (get_element 2 (get_element 1 (car actual_params)))
                                  (M_state_lookup (alias_finder (get_element 1 (get_element 1 (car actual_params))) state state) class_info state instance_closure compile_type break try)
                                  (get_element 1 (get_element 1 (car actual_params)))
                                  (cddr (car actual_params))
                                  class_info state instance_closure compile_type return break try))) (parameter_translator (cdr actual_params) class_info state instance_closure compile_type break try)))
      ; field from an object
      ((and (list? (car actual_params)) (eq? 'dot (get_element 0 (car actual_params))))
       (cons (M_state_instance_field_lookup_shell
        (get_element 2 (car actual_params))
        (get_element 1 (car actual_params))
        class_info state instance_closure compile_type break try) (parameter_translator (cdr actual_params) class_info state instance_closure compile_type break try)))
 
      ; variable
      (else (cons (M_state_lookup (car actual_params) class_info state instance_closure compile_type break try) (parameter_translator (cdr actual_params) class_info state instance_closure compile_type break try)))
      )))
     
; this adds the parameters from the method call to a single sub-list of state             
(define M_state_add_parameters_to_state
  (lambda (fields lis)
    (cond
      ((null? fields) lis)
      (else (cons (cons (box (car fields)) (car (M_state_add_parameters_to_state (cdr fields) lis))) (cdr (M_state_add_parameters_to_state (cdr fields) lis))))
      )))

; this should find the index that the field is at in the class closure field name list.
; Then, we get the remainder in the list and set that to index.
; Then, we lookup that index in the instance field's closure to output the correct value for the field. 
(define M_state_instance_field_lookup_shell
  (lambda (field_name instance_name class_info state instance_closure compile_type break try)
    (cond
      ; if the field name is not super, look it in the state to get its class
      ((not (eq? 'super instance_name))
       (unbox (get_element (field_index_finder field_name (get_element 1 (M_state_get_class_closure_info
                                                                   (get_element 0
                                                                   (M_state_lookup (alias_finder instance_name state state) class_info state instance_closure compile_type break try)) class_info)))
                    (get_element 1 (M_state_lookup (alias_finder instance_name state state) class_info state instance_closure compile_type break try))))) 
      ; if super is the name
      (else (unbox (get_element (field_index_finder field_name
                                (M_state_get_class_closure_info (get_element 0 (M_state_get_class_closure_info (get_element 0 instance_closure) class_info)) class_info))
                         instance_closure)))
      
       )))

(define field_index_finder
  (lambda (field_name class_closure_field_names_list)
    (cond
      ((null? class_closure_field_names_list) (error "field name not found in class closure"))
      ((eq? field_name (car class_closure_field_names_list)) (length (cdr class_closure_field_names_list))) ;might need to change this to without the cdr
      (else (field_index_finder field_name (cdr class_closure_field_names_list)))
      )))

(define M_state_instance_field_lookup_shell_boxed
  (lambda (field_name instance_name class_info state instance_closure compile_type break try)
    (cond
      ; if the field name is not super, look it in the state to get its class
      ((not (eq? 'super instance_name))
       (get_element (field_index_finder field_name (get_element 1 (M_state_get_class_closure_info
                                                                   (get_element 0
                                                                   (M_state_lookup (alias_finder instance_name state state) class_info state instance_closure compile_type break try)) class_info)))
                    (get_element 1 (M_state_lookup (alias_finder instance_name state state) class_info state instance_closure compile_type break try))))
      ; if super is the name
      (else (get_element (field_index_finder field_name
                                (M_state_get_class_closure_info (get_element 0 (M_state_get_class_closure_info (get_element 0 instance_closure) class_info)) class_info))
                         instance_closure))
      
       )))


(define M_state_modify_field_value
  (lambda (field_name instance_closure newval instance_name class_info state compile_type break try)
    (set-box! (M_state_instance_field_lookup_shell_boxed field_name instance_name class_info state instance_closure compile_type break try) newval)
    ))
                                  
   
(define M_state_func_environment
  (lambda (state num)
    (cond
      ((null? num) state)
      ((eq? num (length (get_element 0 state))) state)
      ((< num (length (get_element 0 state))) (M_state_func_environment (list (cdr(get_element 0 state)) (cdr (get_element 1 state)) (get_element 2 state) (get_element 3 state)) num))
      ((> num (length (get_element 0 state))) (error "things are out of sync"))
    )))

(define M_state_create_func_layer
  (lambda (formal_params actual_params new_state)
    (cond
      ((null? formal_params) new_state)
      ((operator? (car actual_params)) new_state)
      ((boolean_operator? (car actual_params)) new_state)
      (else (M_state_create_func_layer
             (cdr formal_params)
             (cdr actual_params)
             (M_state_add_to_func_layer new_state (car formal_params) (car actual_params))
            ))
        )))

(define M_state_add_to_func_layer
  (lambda (new_state formal_param actual_param)
    (list (M_state_add_to_declared_list (get_element 0 new_state) formal_param)
          (M_state_add_to_value_list (get_element 1 new_state) actual_param)
          (get_element 2 new_state)
          (get_element 3 new_state))
    ))

(define M_state_actual_param_evaluator
  (lambda (param class_info state return_list instance_closure compile_type break try)
    (cond
      ((null? param) return_list)
      ((null? (car param)) return_list)
      
      ((number? (car param)) (M_state_actual_param_evaluator (cdr param) class_info state (append return_list (list (car param))) instance_closure compile_type break try))
      ((member? (car param) (get_element 0 state)) (M_state_actual_param_evaluator (cdr param) class_info state (append return_list (list (M_state_lookup (car param) class_info state instance_closure compile_type break try))) instance_closure compile_type break try))
      ((operator? (car param)) (cons (M_value param class_info state instance_closure compile_type break try) return_list))
      ((and (list? (car param)) (boolean_operator? (caar param))) (M_state_actual_param_evaluator (cdr param) class_info state (append return_list (M_boolean param state instance_closure compile_type break try) ) instance_closure compile_type break try))
      ((list? (car param)) (M_state_actual_param_evaluator (cdr param) class_info state (append return_list (M_state_actual_param_evaluator (car param) class_info state return_list instance_closure compile_type break try)) instance_closure compile_type break try))
      ((boolean? (M_boolean_truth_finder (car param) class_info state instance_closure compile_type break try)) (append return_list (list (M_boolean_truth_finder (car param) class_info state instance_closure compile_type break try)))) ;this might need a true/false converter
      )))



(define evaluate
  (lambda (lis class_info state compile_type instance_closure return break try) ;add "instance_closure" to evaluate
    ;(newline) (display "declared list at top of evaluate") (display (get_element 0 state))
    ;(newline) (display "value list at top of evaluate") (display (get_element 1 state))
    ;(newline) (display "lis at top of evaluate") (display lis)
    ;(newline)
    (newline)
    (display class_info)
    (cond
      ((null? lis) (list class_info state))
      ((null? (car lis)) (list class_info state))
      ((eq? 'function (caar lis)) (evaluate (cdr lis) class_info (M_state_function_closure_maker (list (car lis)) class_info state) compile_type instance_closure return break try))

      ; if 'funcall and no parameters
      ((and (eq? 'funcall (caar lis)) (eq? 2 (length (car lis)))) (evaluate (cdr lis) class_info (list (get_element 0 state)
                                                                                                       (M_state_sync_value_list (list (get_element 0 state)
                                                                                                                                      (get_element 1 (get_element 1 (call/cc (lambda (return1)
                                                                                                                                                                               (M_value_call_function
                                                                                                                                                                                (get_element 2 (get_element 1 (car lis)))
                                                                                                                                                                                (get_element 0 (M_state_lookup
                                                                                                                                                                                                (alias_finder
                                                                                                                                                                                                 (get_element 1 (get_element 1 (car lis)))
                                                                                                                                                                                                 state state) class_info state compile_type break try))
                                                                                                                                                                                (get_element 1 (get_element 1 (car lis)))
                                                                                                                                                                                '()
                                                                                                                                                                                class_info state instance_closure compile_type return1 break try)))))
                                                                                                                                      (get_element 2 state)
                                                                                                                                      (get_element 3 state))) 
                                                                                                       (get_element 2 state)
                                                                                                       (get_element 3 state))
                                                                            compile_type
                                                                            (M_state_lookup
                                                                             (alias_finder
                                                                              (get_element 1 (get_element 1 (car lis)))
                                                                              state state) class_info state instance_closure compile_type break try)
                                                                  return break try))

      ;if 'funcall with parameters
      ((eq? 'funcall (caar lis)) (evaluate (cdr lis) class_info (list (get_element 0 state)
                                                                                 (M_state_sync_value_list (list (get_element 0 state)
                                                                                                                (get_element 1 (get_element 1 (call/cc (lambda (return1)
                                                                                                                                          (M_value_call_function
                                                                                                                                           (get_element 2 (get_element 1 (car lis)))
                                                                                                                                           (get_element 0 (M_state_lookup (alias_finder (get_element 1 (get_element 1 (car lis))) state state) class_info state instance_closure compile_type break try))
                                                                                                                                           (get_element 1 (get_element 1 (car lis)))
                                                                                                                                           (cddr (car lis)) ; we THINK this gets the parameter list, neeed cddr b/c it may be a list of parameters
                                                                                                                                           class_info state instance_closure compile_type return1 break try)))))
                                                                                                                (get_element 2 state)
                                                                                                                (get_element 3 state))) 
                                                                                 (get_element 2 state)
                                                                                 (get_element 3 state))
                                                                      compile_type
                                                                      (M_state_lookup
                                                                             (alias_finder
                                                                              (get_element 1 (get_element 1 (car lis)))
                                                                              state state) class_info state instance_closure compile_type break try)
                                                                      return break try))

      ((eq? 'var (caar lis)) (M_state_declaration lis class_info state compile_type instance_closure return break try))
      ((eq? '= (caar lis)) (M_state_assignment lis class_info state compile_type instance_closure return break try))
      ((eq? 'if (caar lis)) (M_state_if lis class_info state compile_type instance_closure return break try))

      ; when it's a "while" and the condition is true
      ((eq? 'while (caar lis))
       (evaluate (cdr lis) class_info 
                (list (get_element 0 state)
                 (M_state_sync_value_list (list (get_element 0 state) (cadr (call/cc (lambda (break) (M_state_while lis class_info state compile_type instance_closure return break try)))) (get_element 2 state) (get_element 3 state)))
                 (get_element 2 state) (get_element 3 state)) compile_type instance_closure return break try))

      ; "return with a 'funcall and no parameters
      ((and (eq? 2 (length lis)) (and (and (list? (cadar lis)) (eq? 'return (caar lis))) (eq? 'funcall (caadar lis))))
       (call/cc (lambda (return1) (M_value_call_function
                                   (get_element 2 (get_element 1 (get_element 1 (car lis)))) ;get's the func_name
                                   (get_element 0 (M_state_lookup (alias_finder (get_element 1 (get_element 1 (get_element 1 (car lis)))) state state) class_info state instance_closure compile_type break try)) ;get's the class_name
                                   (get_element 1 (get_element 1 (get_element 1 (car lis))))
                                   '() class_info state
                                   (M_state_lookup (alias_finder (get_element 1 (get_element 1 (get_element 1 (car lis)))) state state) class_info state instance_closure compile_type break try)
                  instance_closure compile_type return1 break try))))
      
      ;"return" with a "funcall" and yes parameters
      ((and (and (list? (cadar lis)) (eq? 'return (caar lis))) (eq? 'funcall (caadar lis)))
       (call/cc (lambda (return1) (M_value_call_function
                                   (get_element 2 (get_element 1 (get_element 1 (car lis)))) ;get's the func_name
                                   (get_element 0 (M_state_lookup (alias_finder (get_element 1 (get_element 1 (get_element 1 (car lis)))) state state) class_info state instance_closure compile_type break try)) ;get's the class_name
                                   (get_element 1 (get_element 1 (get_element 1 (car lis))))
                                   (cddr (get_element 1 (car lis))) ;we THINK this gets the parameter list, have to do cddr in case it's a list of parameters
                                   class_info state
                          
                                   ;(M_state_lookup (alias_finder (get_element 1 (get_element 1 (get_element 1 (car lis)))) state state) class_info state instance_closure break try)
                  instance_closure compile_type return1 break try))))
                                                                
      
      ; when it's return and a boolean, this gets it to be "true" or "false"
      ((and(eq? 'return (caar lis)) (boolean?
                                     (M_state_lookup 'return class_info (list (get_element 0 state)
                                                                              (M_state_modify_value_list state 'return
                                                                                                         (M_state_return_helper
                                                                                                          (cadar lis)
                                                                                                          class_info state instance_closure compile_type break try)
                                                                                                         break try)
                                                                              (get_element 2 state)
                                                                              (get_element 3 state))
                                                     instance_closure compile_type break try)))
       (return (list
                (get_element 0 state)
                (M_state_modify_value_list state 'return
                                           (M_state_return_helper (cadar lis) class_info state instance_closure compile_type break try)
                                           break try)
                (get_element 2 state)
                (get_element 3 state))))

      ((and (eq? 'return (caar lis)) (number? (cadar lis)))
        (return (list (get_element 0 state) (M_state_modify_value_list state 'return (cadar lis) break try) (get_element 2 state) (get_element 3 state))))
       
      ((eq? 'return (caar lis))
       (return (list (get_element 0 state) (M_state_modify_value_list state 'return (M_state_return_helper (cadar lis) class_info state instance_closure compile_type break try) break try) (get_element 2 state) (get_element 3 state))))

      
      ((eq? 'begin (caar lis)) (evaluate (cdr lis) class_info (list (get_element 0 state)
                                         (M_state_sync_value_list (list (get_element 0 state)
                                                                        (cadr (evaluate (cdar lis) class_info (list (M_state_add_row_to_declared_list (get_element 0 state))
                                                                                                         (M_state_add_row_to_value_list (get_element 1 state))
                                                                                                         (M_state_add_row_to_list (get_element 2 state))
                                                                                                         (M_state_add_row_to_list (get_element 3 state)))
                                                                                        compile_type instance_closure return break try))
                                                                        (get_element 2 state)
                                                                        (get_element 3 state)))
                                          (get_element 2 state)
                                          (get_element 3 state))
                                         compile_type instance_closure return break try))

      ((eq? 'continue (caar lis)) (evaluate '() class_info state compile_type instance_closure return break try))

      ((eq? 'break (caar lis))(break (list (get_element 0 state) (M_state_sync_value_list state) (get_element 2 state) (get_element 3 state))))

      ; hit "try", "finally" doesn't exist, throw is triggered.
      ((and (and (eq? 'try (caar lis)) (null? (get_element 3 (car lis))))  (eq? 2 (length(M_state_try (get_element 1 (car lis)) class_info state compile_type instance_closure return break try))))
       (let ((x (M_state_try (get_element 1 (car lis)) class_info state compile_type instance_closure return break try)))
       (evaluate (cdr lis) class_info (list (get_element 0 state)                
                                        (M_state_sync_value_list (list (get_element 0 state)
                                                                       (cadr (M_state_catch
                                                                              (get_element 2 (car lis)) class_info 
                                                                              (list (get_element 0 state)
                                                                              (M_state_sync_value_list (list (get_element 0 (get_element 0 x))
                                                                                                             (cadar x)
                                                                                                             ; ^ this might be wrong
                                                                                                             (get_element 2 state)
                                                                                                             (get_element 3 state)))
                                                                              (get_element 2 state)
                                                                              (get_element 3 state))
                                                                              (M_value (list (list-ref x 1)) class_info (car x) instance_closure compile_type break try)
                                                                              instance_closure return break try))
                                                                       (get_element 2 state)
                                                                       (get_element 3 state)))                                     
                                        (get_element 2 state)
                                        (get_element 3 state)
                                        compile_type instance_closure return break try))))
      
      ; hit "try", "finally" doesn't exist, throw is not triggered.
      ((and (eq? 'try (caar lis)) (null? (get_element 3 (car lis))))
       (evaluate (cdr lis) class_info (list (get_element 0 state)                                      
                                 (M_state_sync_value_list (list (get_element 0 state)
                                                                (cadr (M_state_try (get_element 1 (car lis)) class_info state compile_type instance_closure return break try))
                                                                (get_element 2 state)
                                                                (get_element 3 state)))       
                                        (get_element 2 state)
                                        (get_element 3 state))
                                        compile_type instance_closure return break try))

       ; hit "try", "finally" does exist, throw is triggered.
      ((and (eq? 'try (caar lis)) (eq? 2 (length(M_state_try (get_element 1 (car lis)) class_info state compile_type instance_closure return break try))))
       (let ((x (M_state_try (get_element 1 (car lis)) class_info state compile_type instance_closure return break try)))
       (evaluate (cdr lis) class_info (list (get_element 0 state)
                 (cadr (M_state_finally (cadr(get_element 3 (car lis))) class_info 
                                        (list (get_element 0 state)
                                        (M_state_sync_value_list
                                         (list (get_element 0 state)
                                               (cadr (M_state_catch (get_element 2 (car lis)) class_info 
                                                                    (list (get_element 0 state)
                                                                          (M_state_sync_value_list (list (get_element 0 state)
                                                                                                         (cadar x)
                                                                                                         (get_element 2 state)
                                                                                                         (get_element 3 state)))
                                                                          (get_element 2 state)
                                                                          (get_element 3 state))
                                                                    (M_value (list (list-ref x 1)) class_info (car x) instance_closure compile_type break try)
                                                                    instance_closure return break try))
                                               (get_element 2 state)
                                               (get_element 3 state)))
                                         (get_element 2 state)
                                         (get_element 3 state)
                                         return break try)
                                        compile_type instance_closure return break try))
                 (get_element 2 state)
                 (get_element 3 state))
                 compile_type instance_closure return break try))) 

      ; hit "try", "finally" does exist, throw is not triggered.
      ((eq? 'try (caar lis))
       (evaluate (cdr lis) class_info (list (get_element 0 state)
                                 (cadr (M_state_finally (cadr (get_element 3 (car lis))) class_info 
                                                        (list (get_element 0 state)                                                                                               
                                                              (M_state_sync_value_list
                                                               (list (get_element 0 state)
                                                                     (cadr (M_state_try
                                                                            (get_element 1 (car lis)) class_info state compile_type instance_closure return break try))
                                                                     (get_element 2 state)
                                                                     (get_element 3 state)))
                                                              (get_element 2 state)
                                                              (get_element 3 state))
                                                              compile_type instance_closure return break try))
                                 (get_element 2 state)
                                 (get_element 3 state)) compile_type instance_closure return break try))

      ((eq? 'throw (caar lis)) (try (list state (cadar lis))))
      
      )))

(define M_state_declaration
  (lambda (lis class_info state compile_type instance_closure return break try)
    (cond
       ;if the first word is "var" and this sublist just has the declaration in it (ex: var x)
      ((eq? '() (cddar lis))
       (evaluate (cdr lis) class_info 
                 (list (M_state_add_to_declared_list (get_element 0 state) (cadar lis))
                       (M_state_add_to_value_list (get_element 1 state) "error")
                       (get_element 2 state)
                       (get_element 3 state))
                 compile_type instance_closure return break try))
      ;if the first word is "var" and the right is a number (ex: var x = 10)
      ((number? (get_element 2 (car lis)))
       (evaluate (cdr lis) class_info 
                 (list (M_state_add_to_declared_list (get_element 0 state) (cadar lis))
                       (M_state_add_to_value_list (get_element 1 state) (get_element 2 (car lis)))
                       (get_element 2 state)
                       (get_element 3 state))
                 compile_type instance_closure return break try))
      ; if the first word is var and the 3rd element is a variable. (ex: var x = y (and y is declared)) 
      ((member? (get_element 2 (car lis)) (get_element 0 state))
       (evaluate (cdr lis) class_info 
                 (list (M_state_add_to_declared_list (get_element 0 state) (cadar lis))
                       (M_state_add_to_value_list (get_element 1 state) (M_state_lookup(get_element 2 (car lis)) class_info state instance_closure compile_type compile_type break try))
                       (get_element 2 state)
                       (get_element 3 state))
                 compile_type instance_closure return break try))
      ;if the first word is "var" and this sublist is a boolean. ex: var x = a && b
      ((boolean_operator? (car (caddar lis)))
       (evaluate (cdr lis) class_info 
                 (list (M_state_add_to_declared_list (get_element 0 state) (cadar lis))
                       (M_state_add_to_value_list (get_element 1 state) (M_boolean_tf_to_truefalse (M_boolean (caddar lis) state instance_closure compile_type break try)))
                       (get_element 2 state)
                       (get_element 3 state))
                 compile_type instance_closure return break try))
      
      ;if the first word is "var" and after the = is a "new" object creation.
      ;this adds the object name to the declared_list and the instance closure to the value_list
      ((eq? 'new (car (caddar lis)))
       (evaluate (cdr lis) class_info
                 (list (M_state_add_to_declared_list (get_element 0 state) (get_element 1 (car lis)))
                       (M_state_add_to_value_list (get_element 1 state) (M_state_instance_closure_maker (get_element 1 (get_element 2 (car lis))) class_info))
                       (get_element 2 state)
                       (get_element 3 state))
                             compile_type instance_closure return break try))

      ;if the first word is "var" and there is more in this sublist than just the declaration aka our first word is var and it's not of the others ex: var x = 5+7
      (else (evaluate (cdr lis) class_info 
                 (list (M_state_add_to_declared_list (get_element 0 state) (cadar lis))
                       (M_state_add_to_value_list (get_element 1 state) (M_value (caddar lis) class_info state instance_closure compile_type break try))
                       (get_element 2 state)
                       (get_element 3 state))
                 compile_type instance_closure return break try))

      
      )))

(define M_state_assignment
  (lambda (lis class_info state compile_type instance_closure return break try)
    (cond
      ; if it's an assignment statement and it's in the declared list, assuming the second value is a list and a boolean ex: x = a && b
      ; ....and the left side has "dot" in it
      ((and (list? (cadar lis)) (and (and (member? (get_element 1 (cadar lis)) (get_element 0 state)) (list? (caddar lis))) (boolean_operator? (car (caddar lis)))))
       (M_state_modify_field_value
        (get_element 2 (cadar lis))
        (M_state_lookup (alias_finder (get_element 1 (cadar lis)) state state)
         class_info state instance_closure compile_type break try)
        (M_boolean_tf_to_truefalse (M_boolean (caddar lis) state instance_closure compile_type break try)) (get_element 1 (get_element 1 (car lis)))  class_info state compile_type break try)
       (evaluate (cdr lis) class_info state compile_type instance_closure return break try))

      ;....and the left side does NOT have "dot" in it
      ((and (and (member? (cadar lis) (get_element 0 state)) (list? (caddar lis))) (boolean_operator? (car (caddar lis))))
       (evaluate (cdr lis) class_info (list (get_element 0 state)
                                 (M_state_modify_value_list state (cadar lis) (M_boolean_tf_to_truefalse (M_boolean (caddar lis) state instance_closure compile_type break try)) break try)
                                 (get_element 2 state)
                                 (get_element 3 state))
                                 compile_type instance_closure return break try))

      ; if it's an assignment statement and it's in the declared list, and the right side has a dot ex: this.y = this.x
      ; ....and the left side has "dot" in it
      ((and (and (list? caddar) (eq? 'dot (get_element 0 (caddar lis)))) (and (list? (cadar lis)) (and (member? (get_element 1 (cadar lis)) (get_element 0 state)))))
       (M_state_modify_field_value
        (get_element 2 (cadar lis))
        (M_state_lookup (alias_finder (get_element 1 (cadar lis)) state state)
         class_info state instance_closure compile_type break try)
        (M_state_instance_field_lookup_shell
         (get_element 2 (caddar lis))
         (get_element 1 (caddar lis))
         class_info state instance_closure compile_type break try) (get_element 1 (get_element 1 (car lis))) class_info state compile_type break try)
       (evaluate (cdr lis) class_info state compile_type instance_closure return break try))
      
      ;....and the left side does NOT have "dot" in it ex: y = this.x
       ((and (and (list? caddar) (eq? 'dot (get_element 0 (caddar lis)))) (and (member? (cadar lis) (get_element 0 state)) (member? (caddar lis) (get_element 0 state))))
       (evaluate (cdr lis) class_info (list (get_element 0 state)
                                 (M_state_modify_value_list state (cadar lis)
                                                            (M_state_instance_field_lookup_shell (get_element 2 (caddar lis)) (get_element 1 (caddar lis))
                                                                                                 class_info state instance_closure compile_type break try) break try)
                                 (get_element 2 state)
                                 (get_element 3 state))
                                 compile_type instance_closure return break try))
      
      ; if it's an assignment statement and it's in the declared list, assuming the second value is a list and not a boolean ex: x = 5+7
      ; ....and the left side has "dot" in it
      ((and (list? (cadar lis)) (and (member? (get_element 1 (cadar lis)) (get_element 0 state)) (list? (caddar lis))))
       (M_state_modify_field_value
        (get_element 2 (cadar lis))
        (M_state_lookup (alias_finder (get_element 1 (cadar lis)) state state)
         class_info state instance_closure compile_type break try)
        (M_value (caddar lis) class_info state instance_closure compile_type break try) (get_element 1 (get_element 1 (car lis))) class_info state compile_type break try)
       (evaluate (cdr lis) class_info state compile_type instance_closure return break try))
      
      ;....and the left side does NOT have "dot" in it
      ((and (member? (cadar lis) (get_element 0 state)) (list? (caddar lis)))
       (evaluate (cdr lis) class_info (list (get_element 0 state)
                 (M_state_modify_value_list state (cadar lis) (M_value (caddar lis) class_info state instance_closure compile_type break try) break try)
                 (get_element 2 state)
                 (get_element 3 state))
                 compile_type instance_closure return break try))
      
      ; if it's an assignment statement and it's in the declared list, assuming the second value is a number ex: x = 5
      ; ....and the left side has "dot" in it
      ((and (list? (cadar lis)) (and (member? (get_element 1 (cadar lis)) (get_element 0 state)) (number? (caddar lis))))
       (M_state_modify_field_value
        (get_element 2 (cadar lis))
        (M_state_lookup (alias_finder (get_element 1 (cadar lis)) state state)
         class_info state instance_closure compile_type break try)
        (caddar lis) (get_element 1 (get_element 1 (car lis))) class_info state compile_type break try)
       (evaluate (cdr lis) class_info state compile_type instance_closure return break try))
      
      ;....and the left side does NOT have "dot" in it
      ((and (member? (cadar lis) (get_element 0 state)) (number? (caddar lis)))
       (evaluate (cdr lis) class_info (list (get_element 0 state)
                                 (M_state_modify_value_list state (cadar lis) (caddar lis) break try)
                                 (get_element 2 state)
                                 (get_element 3 state))
                                 compile_type instance_closure return break try))

      
      ; if it's an assignment statement and it's in the declared list, assuming the second value is a variable ex: x = y
      ; ....and the left side has "dot" in it
      ((and (list? (cadar lis)) (and (member? (get_element 1 (cadar lis)) (get_element 0 state)) (member? (caddar lis) (get_element 0 state))))
       (M_state_modify_field_value
        (get_element 2 (cadar lis))
        (M_state_lookup (alias_finder (get_element 1 (cadar lis)) state state)
         class_info state instance_closure compile_type break try)
        (M_state_lookup (caddar lis) class_info state instance_closure compile_type break try) (get_element 1 (get_element 1 (car lis))) class_info state compile_type break try)
       (evaluate (cdr lis) class_info state compile_type instance_closure return break try))
      
      ;....and the left side does NOT have "dot" in it
       ((and (member? (cadar lis) (get_element 0 state)) (member? (caddar lis) (get_element 0 state)))
       (evaluate (cdr lis) class_info (list (get_element 0 state)
                                 (M_state_modify_value_list state (cadar lis) (M_state_lookup (caddar lis) class_info state instance_closure compile_type break try) break try)
                                 (get_element 2 state)
                                 (get_element 3 state))
                                 compile_type instance_closure return break try))
      
      ; if it's an assignment statement and gets to this line, it's not in the declared list and should fail. 
      (else (error "our version of variable not initialized"))
      )))

(define M_state_if
  (lambda (lis class_info state compile_type instance_closure return break try)
    (cond
       ; if the list starts with "if" and the condition next to it is true...then evalute it (we think this one is right) 
      ((M_boolean (cadar lis) state compile_type break try)
       (evaluate (cdr lis) class_info 
                 (let ((x (evaluate (list (get_element 2 (car lis))) class_info state compile_type instance_closure return break try)))
                 (list (car x)
                 (cadr x)
                 (get_element 2 state)
                 (get_element 3 state)))
                 compile_type instance_closure return break try))
      
      ; if the condition is not true, the list has 4 elements and it starts with an "if"
      ((eq? (length (car lis)) 4)
       (evaluate (cdr lis) class_info 
                 (let ((x (evaluate (list (get_element 3 (car lis))) class_info state compile_type instance_closure return break try)))
                 (list (car x)
                 (cadr x)
                 (get_element 2 state)
                 (get_element 3 state)))
                 compile_type instance_closure return break try))
      
       ; if the list starts with "if", but the condition is not true
      (else (evaluate (cdr lis) class_info state compile_type instance_closure return break try))
    )))

(define M_state_while
  (lambda (lis class_info state compile_type instance_closure return break try)
    (cond
      ((M_boolean (cadar lis) state instance_closure compile_type break try)
       (let ((x (evaluate (cddar lis) class_info state compile_type instance_closure return break try)))
       (M_state_while lis class_info (list (car x)
                                (cadr x)
                                (get_element 2 state)
                                (get_element 3 state))
                                compile_type return break try)))
      (else state)
    )))

    
(define M_state_try
  (lambda (lis class_info state compile_type instance_closure return break try)
      (call/cc (lambda (try) (evaluate lis class_info state compile_type instance_closure return break try)))
    ))

(define M_state_catch
  (lambda (lis class_info state thrown_value compile_type instance_closure return break try)
      (evaluate (get_element 2 lis) class_info 
                (list (M_state_add_to_declared_list (M_state_add_row_to_declared_list (get_element 0 state)) (car (get_element 1 lis)))
                (M_state_add_to_value_list (M_state_add_row_to_value_list (get_element 1 state)) thrown_value)
                (M_state_add_row_to_list (get_element 2 state))
                (M_state_add_row_to_list (get_element 3 state)))
                compile_type instance_closure return break try)
    ))

(define M_state_finally
  (lambda (lis class_info state compile_type instance_closure return break try)
    (evaluate lis class_info state compile_type instance_closure return break try)
     ))
           
(define M_state_sync_value_list
  (lambda (state)
    (cond
      ((eq? (length (get_element 0 state)) (length (get_element 1 state))) (get_element 1 state))
      (else (M_state_sync_value_list (list (get_element 0 state)
                                           (M_state_remove_top_layer_value_list (get_element 1 state))
                                           (get_element 2 state)
                                           (get_element 3 state))))
      )))
                        


(define M_state_add_row_to_declared_list
  (lambda declared_list 
    (cons '() (car declared_list))
    ))

(define M_state_add_row_to_value_list
  (lambda value_list  
    (cons '() (car value_list))
    ))

(define M_state_add_row_to_list
  (lambda lis
    (cons '() (car lis))
    ))

(define M_state_add_to_declared_list
  (lambda (declared_list var)
    (cons (cons (box var) (car declared_list)) (cdr declared_list))
    ))

(define M_state_add_to_func_name_list
  (lambda (func_name_list var)
    (cons (cons (box var) (car func_name_list)) (cdr func_name_list))
    ))

(define M_state_add_to_func_closure_list
  (lambda (func_closure_list var)
    (cons (cons var (car func_closure_list)) (cdr func_closure_list))
    ))

(define M_state_remove_top_layer_declared_list
  (lambda (declared_list)
    (cdr declared_list)
    ))

(define M_state_remove_top_layer_value_list
  (lambda (value_list)
    (cdr value_list)
    ))

(define M_state_add_to_value_list
  (lambda (value_list val)
    (cons (cons (box val) (car value_list)) (cdr value_list))
       ))

;if the variable is already in the declared-list, then this changes the value in the value list.
(define M_state_modify_value_list
  (lambda (state var newval break try)
    (cond
      ((null? (get_element 0 state)) (error "our version of variable not initialized"))
      ((and (list? (car (get_element 0 state))) (member? var (car (get_element 0 state))))
       (cons (M_state_modify_value_list (list (car (get_element 0 state))
                                              (car (get_element 1 state))
                                              (get_element 2 state)
                                              (get_element 3 state))
                                        var newval break try) (cdr (get_element 1 state))))
      
      ((list? (car (get_element 0 state)))
       (cons (car (get_element 1 state)) (M_state_modify_value_list (list (cdr (get_element 0 state))
                                                                          (cdr (get_element 1 state))
                                                                          (get_element 2 state)
                                                                          (get_element 3 state))
                                                                          var newval break try)))
      
     
      ((eq? var (unbox (car (get_element 0 state)))) (set-box! (car (get_element 1 state)) newval) (get_element 1 state))   
      (else (cons (car (get_element 1 state)) (M_state_modify_value_list (list (cdr (get_element 0 state))
                                                                               (cdr (get_element 1 state))
                                                                               (get_element 2 state)
                                                                               (get_element 3 state))
                                                                         var newval break try)))
    )))

(define M_state_lookup
  (lambda (var class_info state instance_closure compile_type break try)
    (cond
      ((null? (get_element 0 state)) (error "variable not found"))
      ((eq? 'super var) (list
                         compile_type
                         (cadr instance_closure)))
      
                              
      ; if the var is a list, that means it has "new" in it. If so, return the closure (that we make right now)
      ((list? var) (M_state_instance_closure_maker (get_element 1 var) class_info))
      ;((eq? 'this var) (M_state_lookup_this class_info state state instance_closure break try))
      ((and (and (list? (car (get_element 0 state))) (member? var (car (get_element 0 state)))) (not(null? (car (get_element 0 state)))))
       (M_state_lookup var class_info (list (car (get_element 0 state))
                                 (car (get_element 1 state))
                                 (get_element 2 state)
                                 (get_element 3 state)) instance_closure compile_type break try))
      
      ((list? (car (get_element 0 state))) (M_state_lookup var class_info (list (cdr (get_element 0 state))
                                                                     (cdr (get_element 1 state))
                                                                     (get_element 2 state)
                                                                     (get_element 3 state)) instance_closure compile_type break try))
      
      ((eq? var (unbox (car (get_element 0 state)))) (variable_type? (unbox (car (get_element 1 state))) class_info break try))
      (else (M_state_lookup var class_info (list (cdr (get_element 0 state))
                                      (cdr (get_element 1 state))
                                      (get_element 2 state)
                                      (get_element 3 state)) instance_closure compile_type break try))
      )))


(define M_value
  (lambda (expression class_info state instance_closure compile_type break try)
    (cond
      ((null? expression) '())
      ((number? expression) expression)
      ((number? (car expression)) (car expression))

      ; if the expression is this.x
      ((and (list? (get_element 0 expression)) (eq? 'dot (get_element 0 (get_element 0 expression))))
       (M_state_instance_field_lookup_shell
        (get_element 2 (get_element 0 expression))
        (get_element 1 (get_element 0 expression))
        class_info state instance_closure compile_type break try))
       
      
      ((member? (get_element 0 expression) (get_element 0 state))
       (M_state_lookup (get_element 0 expression) class_info state instance_closure compile_type break try))
      
      ((and (and (eq? (length expression) 2) (eq? (car expression) '-))) 
       (* -1 (M_value (get_element 1 expression) class_info state instance_closure compile_type break try)))
       
      ((and (and (eq? (car expression) '+) (number? (get_element 1 expression))) (number? (get_element 2 expression)))
       (+ (get_element 1 expression) (get_element 2 expression)))
      
      ((and (and (eq? (car expression) '-) (number? (get_element 1 expression))) (number? (get_element 2 expression)))
       (- (get_element 1 expression) (get_element 2 expression)))

     
      
      ((and (and (eq? (car expression) '*) (number? (get_element 1 expression))) (number? (get_element 2 expression)))
       (* (get_element 1 expression) (get_element 2 expression)))
      
      ((and (and (eq? (car expression) '/) (number? (get_element 1 expression))) (number? (get_element 2 expression)))
       (/ (get_element 1 expression) (get_element 2 expression)))
      
      ((and (and (eq? (car expression) '%) (number? (get_element 1 expression))) (number? (get_element 2 expression)))
       (remainder (get_element 1 expression) (get_element 2 expression)))

      ;these are for "funcall"'s that don't have actual parameters 
      ((and (eq? 2 (length expression)) (eq? 'funcall (get_element 0 expression)))
       (M_state_lookup 'return class_info (call/cc (lambda (return1) (M_value_call_function
                                                                      (get_element 2 (get_element 1 expression))
                                                                      (get_element 0 (M_state_lookup (alias_finder (get_element 1 (get_element 1 expression)) state state) class_info state instance_closure compile_type break try))
                                                                      (get_element 1 (get_element 1 expression))
                                                                      '()
                                                                      class_info state instance_closure compile_type return1 break try))) instance_closure compile_type break try))
      
      ((and (and (list? (get_element 1 expression)) (eq? 2 (length (get_element 1 expression)))) (eq? 'funcall (car (get_element 1 expression))))
       (M_value (list (get_element 0 expression)
                      (M_state_lookup 'return class_info (call/cc (lambda (return1) (M_value_call_function
                                                                                     (get_element 2 (get_element 1 (get_element 1 expression)))
                                                                                     (get_element 0 (M_state_lookup (alias_finder (get_element 1 (get_element 1 (get_element 1 expression))) state state) class_info state instance_closure compile_type break try))
                                                                                     (get_element 1 (get_element 1 (get_element 1 expression)))
                                                                                     '()
                                                                                     class_info state instance_closure compile_type return1 break try))) instance_closure compile_type break try)
                      (get_element 2 expression)) class_info state instance_closure compile_type break try))

      ((and (and (list? (get_element 2 expression)) (eq? 2 (length (get_element 2 expression)))) (eq? 'funcall (car (get_element 2 expression))))
       (M_value (list (get_element 0 expression)
                      (get_element 1 expression)
                      (M_state_lookup 'return class_info (call/cc (lambda (return1) (M_value_call_function
                                                                                     (get_element 2(get_element 1 (get_element 2 expression)))
                                                                                     (get_element 0 (M_state_lookup (alias_finder (get_element 1 (get_element 1 (get_element 2 expression))) state state) class_info state instance_closure compile_type break try))
                                                                                     (get_element 1 (get_element 1 (get_element 2 expression)))
                                                                                     '()
                                                                                     class_info state instance_closure compile_type return1 break try))) instance_closure compile_type break try)) class_info state instance_closure compile_type break try))

      ; the rest of these "funcall" ones are if there are actual params passed in
      ((eq? 'funcall (get_element 0 expression))
       (M_state_lookup 'return class_info (call/cc (lambda (return1) (M_value_call_function
                                                                      (get_element 2 (get_element 1 expression))
                                                                      (get_element 0 (M_state_lookup (alias_finder (get_element 1 (get_element 1 expression)) state state) class_info state instance_closure compile_type break try))
                                                                      (get_element 1 (get_element 1 expression))
                                                                      (cddr expression) 
                                                                      class_info state instance_closure compile_type return1 break try))) instance_closure compile_type break try))
      
      ((and (list? (get_element 1 expression)) (eq? 'funcall (car (get_element 1 expression))))
       (M_value (list (get_element 0 expression)
                      (M_state_lookup 'return class_info (call/cc (lambda (return1) (M_value_call_function
                                                                                     (get_element 2 (get_element 1 (get_element 1 expression)))
                                                                                     (get_element 0 (M_state_lookup (alias_finder (get_element 1 (get_element 1 (get_element 1 expression))) state state) class_info state instance_closure compile_type break try))
                                                                                     (get_element 1 (get_element 1 (get_element 1 expression)))
                                                                                     (list (cddr (get_element 1 expression)))
                                                                                     class_info state instance_closure compile_type return1 break try))) instance_closure compile_type break try)
                      (get_element 2 expression)) class_info state instance_closure compile_type break try))

      ((and (list? (get_element 2 expression)) (eq? 'funcall (car (get_element 2 expression))))
       (M_value (list (get_element 0 expression)
                      (get_element 1 expression)
                      (M_state_lookup 'return class_info (call/cc (lambda (return1) (M_value_call_function
                                                                                     (get_element 2(get_element 1 (get_element 2 expression)))
                                                                                     (get_element 0 (M_state_lookup (alias_finder (get_element 1 (get_element 1 (get_element 2 expression))) state state) class_info state instance_closure compile_type break try))
                                                                                     (get_element 1 (get_element 1 (get_element 2 expression)))
                                                                                     (list (cddr (get_element 2 expression)))
                                                                                     class_info state instance_closure compile_type return1 break try))) instance_closure compile_type break try)) class_info state instance_closure compile_type break try))
      ; the left side of the equation is a field (aka as "dot")
      ((and (list? (get_element 1 expression)) (eq? 'dot (get_element 0 (get_element 1 expression))))
       (M_value (list (get_element 0 expression)
                      (M_state_instance_field_lookup_shell
                       (get_element 2 (get_element 1 expression))
                       (get_element 1 (get_element 1 expression))
                       class_info state instance_closure compile_type break try)
                         (get_element 2 expression)) class_info state instance_closure compile_type break try))

      ; the right side of the equation is a field (aka as "dot")
      ((and (list? (get_element 2 expression)) (eq? 'dot (get_element 0 (get_element 2 expression))))
       (M_value (list (get_element 0 expression)
                              (get_element 1 expression)
                              (M_state_instance_field_lookup_shell
                               (get_element 2 (get_element 2 expression))
                               (get_element 1 (get_element 2 expression))
                               class_info state instance_closure compile_type break try))
                class_info state instance_closure compile_type break try))
      
      ((list? (get_element 1 expression))
       (M_value (list (get_element 0 expression)
                             (M_value (get_element 1 expression) class_info state instance_closure compile_type break try) (get_element 2 expression)) class_info state instance_closure compile_type break try))

      ((list? (get_element 2 expression))
       (M_value (list (get_element 0 expression)
                              (get_element 1 expression)  (M_value (get_element 2 expression) class_info state instance_closure compile_type break try)) class_info state instance_closure compile_type break try))

      ; the left side of the equation is a variable and NOT a "dot"
      ((member? (get_element 1 expression) (get_element 0 state))
       (M_value (list (get_element 0 expression)
                             (M_state_lookup (get_element 1 expression) class_info state instance_closure compile_type break try) (get_element 2 expression)) class_info state instance_closure compile_type break try))

      ; the right side of the equation is a variable and NOT a "dot"
      ((member? (get_element 2 expression) (get_element 0 state))
       (M_value (list (get_element 0 expression)
                              (get_element 1 expression)  (M_state_lookup (get_element 2 expression) class_info state instance_closure compile_type break try)) class_info state instance_closure compile_type break try))
      )))



(define M_boolean
  (lambda (expression class_info state instance_closure compile_type break try)
    (cond
      ((null? expression) false)
      ((eq? expression 'true) #t)
      ((eq? expression 'false) #f)
      ((and (and (list? (get_element 1 expression)) (eq? (car expression) '!)) (boolean_operator? (car (get_element 1 expression))))
       (M_boolean (list (get_element 0 expression) (M_boolean (get_element 1 expression) class_info state instance_closure compile_type break try)) class_info state instance_closure compile_type break try))
      ((eq? (car expression) '!) (M_boolean_tf_to_hashtags (not (M_boolean_truth_finder (get_element 1 expression) class_info state instance_closure compile_type break try))))
      
      ((boolean? expression) expression)
     
      ;Checks if left side is a boolean equation or a integer and calcs it
      ((eq? (get_element 1 expression) 'true) (M_boolean (list (get_element 0 expression) #t (get_element 2 expression)) class_info state instance_closure compile_type break try))
      ((eq? (get_element 1 expression) 'false) (M_boolean (list (get_element 0 expression) #f (get_element 2 expression)) class_info state instance_closure compile_type break try))
      ((and (list? (get_element 1 expression)) (boolean_operator? (car (get_element 1 expression))))
       (M_boolean (list (get_element 0 expression) (M_boolean (get_element 1 expression) class_info state instance_closure compile_type break try) (get_element 2 expression)) class_info state instance_closure compile_type break try))
      ((list? (get_element 1 expression))
       (M_boolean (list (get_element 0 expression) (M_value (get_element 1 expression) class_info state instance_closure compile_type break try) (get_element 2 expression)) class_info state instance_closure compile_type break try))

      ; as above but for right side
      ((eq? (get_element 2 expression) 'true) (M_boolean (list (get_element 0 expression) (get_element 1 expression) #t) class_info state instance_closure compile_type break try))
      ((eq? (get_element 2 expression) 'false) (M_boolean (list (get_element 0 expression) (get_element 1 expression) #f) class_info state instance_closure compile_type break try))
      ((and (list? (get_element 2 expression)) (boolean_operator? (car (get_element 2 expression))))
       (M_boolean (list (get_element 0 expression) (get_element 1 expression) (M_boolean (get_element 2 expression) class_info state instance_closure compile_type break try)) class_info state instance_closure compile_type break try))
      ((list? (get_element 2 expression))
       (M_boolean (list (get_element 0 expression) (get_element 1 expression) (M_value (get_element 2 expression) class_info state instance_closure compile_type break try)) class_info state instance_closure compile_type break try))

      
      ((eq? (car expression) '&&)
       (M_boolean_tf_to_hashtags (and (M_boolean_truth_finder (get_element 1 expression) class_info state instance_closure compile_type break try) (M_boolean_truth_finder (get_element 2 expression) class_info state instance_closure compile_type break try))))
      ((eq? (car expression) '||)
       (M_boolean_tf_to_hashtags (or (M_boolean_truth_finder (get_element 1 expression) class_info state instance_closure compile_type break try) (M_boolean_truth_finder (get_element 2 expression) class_info state instance_closure compile_type break try))))

       ; if the left or right element is not a number, look up the value of the variable.
      ;.... and it's a "dot"
      ((and (list? (get_element 1 expression)) (eq? 'dot (get_element 0 (get_element 1 expression))))
       (M_boolean (list
                   (get_element 0 expression)
                   (M_state_instance_field_lookup_shell
                    (get_element 2 (get_element 1 expression))
                    (get_element 1 (get_element 1 expression))
                    class_info state instance_closure compile_type break try)
                   (get_element 2 expression))
                  class_info state instance_closure compile_type break try))

      ((and (list? (get_element 2 expression)) (eq? 'dot (get_element 0 (get_element 2 expression))))
       (M_boolean (list
                   (get_element 0 expression)
                   (get_element 1 expression)
                   (M_state_instance_field_lookup_shell
                    (get_element 2 (get_element 2 expression))
                    (get_element 1 (get_element 2 expression))
                    class_info state instance_closure compile_type break try))
                  class_info state instance_closure compile_type break try))
      
      ; if the left or right element is not a number, look up the value of the variable.
      ;,....and it's not a "dot"
      ((not(number? (get_element 1 expression)))
       (M_boolean (list
                   (get_element 0 expression)
                   (M_state_lookup
                    (get_element 1 expression)
                    class_info state instance_closure compile_type break try)
                   (get_element 2 expression))
                  class_info state instance_closure compile_type break try))
      
      ((not(number? (get_element 2 expression)))
       (M_boolean (list
                   (get_element 0 expression)
                   (get_element 1 expression)
                   (M_state_lookup
                    (get_element 2 expression)
                    class_info state instance_closure compile_type break try))
                  class_info state instance_closure compile_type break try))


      ((eq? (car expression) '==) (M_boolean_equal (get_element 1 expression) (get_element 2 expression)))
      ((eq? (car expression) '!=) (not (M_boolean_equal (get_element 1 expression) (get_element 2 expression))))
      ((eq? (car expression) '<) (< (get_element 1 expression) (get_element 2 expression)))
      ((eq? (car expression) '>) (> (get_element 1 expression) (get_element 2 expression)))
      ((eq? (car expression) '<=) (<= (get_element 1 expression) (get_element 2 expression)))
      ((eq? (car expression) '>=) (>= (get_element 1 expression) (get_element 2 expression)))

      
      
    )))

(define M_boolean_truth_finder
  (lambda (var class_info state instance_closure compile_type break try)
    (cond
      ((null? var) #f)
      ((boolean? var) var)
      ((and (list? var) (eq? 'dot (car var)))
       (M_boolean_tf_to_hashtags (M_state_instance_field_lookup_shell (get_element 2 var) (get_element 1 var) class_info state instance_closure compile_type break try)))
      ((member? var (get_element 0 state))
       (M_boolean_tf_to_hashtags(M_state_lookup var class_info state instance_closure compile_type break try)))
      ((eq? var 'true) #t)
      ((eq? var 'false) #f)
      (else (error "our version of variable not initialized"))
      )))

(define M_boolean_equal
  (lambda (left right)
    (cond
      ((= left right) #t)
      (else #f))))


    
(define M_boolean_truefalse_converter
  (lambda (var)
    (cond
      ((eq? var 'true) #t)
      ((eq? var #t) 'true)
      ((eq? var 'false) #f)
      ((eq? var #f) 'false)
      )))

(define M_boolean_tf_to_truefalse
  (lambda (var)
    (cond
      ((eq? var 'true) 'true)
      ((eq? var #t) 'true)
      ((eq? var 'false) 'false)
      ((eq? var #f) 'false)
      )))

(define M_boolean_tf_to_hashtags
  (lambda (var)
    (cond
      ((eq? var 'true) #t)
      ((eq? var #t) #t)
      ((eq? var 'false) #f)
      ((eq? var #f) #f)
      )))


; variable_type tells us if the variable is declared or not. If it's 'true' or 'false' it converts it to a hashtag
(define variable_type?
  (lambda (var class_info break try)
    (cond
      ((null? var) '())
      ((and (list? var) (null? (car var))) '())
      ((and (list? var) (number? (car var))) (car var))
      ((number? var) var)
      ((and (list? var) (boolean? (car var))) (car var))
      ((boolean? var) var)
      ((and (list? var) (or (eq? 'true (car var))) (eq? 'false (car var))) (M_boolean_tf_to_hashtags (car var)))
      ((or (eq? 'true var) (eq? 'false var)) (M_boolean_tf_to_hashtags var))
      ; if you are a list and the first item is a member of the list of class names
      ((and (list? var) (member? (car var) (get_element 0 class_info))) var)
      (else (display var) (error "our version of variable not initialized"))
      )))

(define member?
  (lambda (x lis)
    (cond
      ((null? lis) #f)
      ((list? (car lis)) (or (member? x (car lis)) (member? x (cdr lis))))
      ((and (box? (car lis)) (eq? x (unbox (car lis)))) #t) 
      ((eq? x (car lis)) #t)
      (else (member? x (cdr lis))))))

(define boolean_operator?
  (lambda (var)
    (cond
      ((member? var (list '== '!= '< '> '<= '>= '&& '|| '!)) #t)
      (else #f)
      )))

(define operator?
  (lambda (var)
    (cond
      ((member? var (list '+ '- '* '/ '%)) #t)
      (else #f)
      )))


; if what we have is just a list
; element 0 = car
; element 1 = cadr
; element 2 = caddr
; element 3 = cadddr

(define get_element
  (lambda (index lis);first item = 0
    (cond
      ((null? lis) '())
      ((eq? index 0) (car lis))
      ((eq? index 1) (cadr lis))
      ((eq? index 2) (caddr lis))
      ((eq? index 3) (cadddr lis))
      (else (get_element (- index 1) (cdr lis)))
      )))

(define M_state_return_helper
  (lambda (var class_info state instance_closure compile_type break try)
    (cond
      ((null? var) 'null)
      ((number? var) var)
      ((and (list? var) (eq? 'dot (car var)))
       (M_state_instance_field_lookup_shell (get_element 2 var) (get_element 1 var) class_info state instance_closure compile_type break try))
      ((member? var (get_element 0 state))
       (M_state_lookup var class_info state instance_closure compile_type break try))  
      ((and (list? var) (boolean_operator? (car var))) (M_boolean var class_info state instance_closure compile_type break try))
      ((list? var) (M_value var class_info state instance_closure compile_type break try))
      ((eq? 'false var) var)
      ((eq? 'true var) var)
      (else (error "invalid return"))
       )))

(define tests
  (lambda x
      (cond
        ((not (eq? (interpret "Unit Tests/fileToParseTest2-1.txt") 20)) (error "Test 2-1 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest2-2.txt") 164)) (error "Test 2-2 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest2-3.txt") 32)) (error "Test 2-3 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest2-4.txt") 2)) (error "Test 2-4 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest2-6.txt") 25)) (error "Test 2-6 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest2-7.txt") 21)) (error "Test 2-7 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest2-8.txt") 6)) (error "Test 2-8 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest2-9.txt") -1)) (error "Test 2-9 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest2-10.txt") 789)) (error "Test 2-10 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest2-14.txt") 12)) (error "Test 2-14 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest2-15.txt") 125)) (error "Test 2-15 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest2-16.txt") 110)) (error "Test 2-16 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest2-17.txt") 2000400)) (error "Test 2-17 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest2-18.txt") 101)) (error "Test 2-18 failed"))
        (display "all tests passed")
        )))
;(tests)


(define tests3
  (lambda x
      (cond
        ((not (eq? (interpret "Unit Tests/fileToParseTest3-1.txt") 10)) (error "Test 3-1 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest3-2.txt") 14)) (error "Test 3-2 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest3-3.txt") 45)) (error "Test 3-3 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest3-4.txt") 55)) (error "Test 3-4 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest3-5.txt") 1)) (error "Test 3-5 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest3-6.txt") 115)) (error "Test 3-6 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest3-7.txt") 'true)) (error "Test 3-7 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest3-8.txt") 20)) (error "Test 3-8 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest3-9.txt") 24)) (error "Test 3-9 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest3-10.txt") 2)) (error "Test 3-10 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest3-11.txt") 35)) (error "Test 3-11 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest3-13.txt") 90)) (error "Test 3-13 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest3-14.txt") 69)) (error "Test 3-14 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest3-15.txt") 87)) (error "Test 3-15 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest3-18.txt") 125)) (error "Test 3-18 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest3-19.txt") 100)) (error "Test 3-19 failed"))
       ; ((not (eq? (interpret "Unit Tests/fileToParseTest3-20.txt") 2000400)) (error "Test 2-18 failed"))
        
        (display "all tests passed")
        )))
;(tests3)

(define tests4
  (lambda x
      (cond
        ((not (eq? (interpret "Unit Tests/fileToParseTest4-1.txt" 'A) 15)) (error "Test 4-1 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest4-2.txt" 'A) 12)) (error "Test 4-2 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest4-3.txt" 'A) 125)) (error "Test 4-3 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest4-4.txt" 'A) 36)) (error "Test 4-4 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest4-5.txt" 'A) 54)) (error "Test 4-5 failed"))
        ((not (eq? (interpret "Unit Tests/fileToParseTest4-6.txt" 'A) 110)) (error "Test 4-6 failed"))
        ;((not (eq? (interpret "Unit Tests/fileToParseTest4-7.txt" 'C) 26)) (error "Test 4-7 failed"))
        ;((not (eq? (interpret "Unit Tests/fileToParseTest4-8.txt" 'Square) 117)) (error "Test 4-8 failed"))
        ;((not (eq? (interpret "Unit Tests/fileToParseTest4-9.txt" 'Square) 32)) (error "Test 4-9 failed"))
        ;((not (eq? (interpret "Unit Tests/fileToParseTest4-10.txt" 'List) 15)) (error "Test 4-10 failed"))
        ;((not (eq? (interpret "Unit Tests/fileToParseTest4-11.txt" 'List) 123456)) (error "Test 4-11 failed"))
        ;((not (eq? (interpret "Unit Tests/fileToParseTest4-12.txt" 'List) 5285)) (error "Test 4-12 failed"))
        ;((not (eq? (interpret "Unit Tests/fileToParseTest4-13.txt" 'C) -716)) (error "Test 4-13 failed"))
        (display "all tests passed")
        )))

(tests4)

;(interpret "Unit Tests/fileToParseTest4-7.txt" 'C)
;(interpret "Unit Tests/fileToParseTest4-9.txt" 'Square)



;(interpret "Unit Tests/fileToParseTest3-20.txt")
;(interpret "Unit Tests/fileToParseTest3-5.txt")
;(interpret "Unit Tests/fileToParseTest3-6.txt")

; These below are unit tests of Part 1
; Note that the ones that end in "true" or "false" aren't returning #t correctly

#|
(eq? (interpret "Unit Tests/fileToParseTest1-1.txt") 150)
(eq? (interpret "Unit Tests/fileToParseTest1-2.txt") -4)
(eq? (interpret "Unit Tests/fileToParseTest1-3.txt") 10)
(eq? (interpret "Unit Tests/fileToParseTest1-4.txt") 16)
(eq? (interpret "Unit Tests/fileToParseTest1-5.txt") 220)
(eq? (interpret "Unit Tests/fileToParseTest1-6.txt") 5)
(eq? (interpret "Unit Tests/fileToParseTest1-7.txt") 6)
(eq? (interpret "Unit Tests/fileToParseTest1-8.txt") 10)
(eq? (interpret "Unit Tests/fileToParseTest1-9.txt") 5)
(eq? (interpret "Unit Tests/fileToParseTest1-10.txt") -39)
;test 11,12,13 should fail
;(interpret "Unit Tests/fileToParseTest1-11.txt")
;(interpret "Unit Tests/fileToParseTest1-12.txt")
;(interpret "Unit Tests/fileToParseTest1-13.txt")

(eq? (interpret "Unit Tests/fileToParseTest1-14.txt") 30)
(eq? (interpret "Unit Tests/fileToParseTest1-15.txt") "'true")
(eq? (interpret "Unit Tests/fileToParseTest1-16.txt") 100)
(eq? (interpret "Unit Tests/fileToParseTest1-17.txt") "'false")
(eq? (interpret "Unit Tests/fileToParseTest1-18.txt") "'true")
(eq? (interpret "Unit Tests/fileToParseTest1-19.txt") 128)
(eq? (interpret "Unit Tests/fileToParseTest1-20.txt") 12)
|#

